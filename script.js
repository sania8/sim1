...
function animate() {
  ...
  if (mode === "first") 
  {
    ...
    text.innerHTML = `<b>Copenhagen Interpreation using quantum particles:</b> This is the pattern observed with quantum partciles that pass through Slit1, <br> 𝝍 describes the probability density of finding a particle at a particular point in space and time.  <br> Observed pattern : |𝝍<sub>1</sub>|<sup>2</sup><br> This is the  observed  pattern when slit1 is open which produces a single slit diffraction pattern .
    where : 𝝍 is the function of space and time. `
   ...
   else if (mode === "second")
    {
      ...
      text.innerHTML = `<b>Copenhagen Interpreation using quantum particles:</b> This is the pattern observed with quantum partciles that pass through Slit2 , <br> 𝝍 describes the probability density of finding a particle at a particular point in space and time.  <br> Observed pattern : |𝝍<sub>2</sub>|<sup>2</sup><br> This is the  observed pattern  when  slit2 is open which produces a single slit diffraction pattern 
      where : 𝝍 is the function of space and time  `
      text.style.textAlign = "center";
...
function animation() {
  ...
  for (let i = 0; i < 10; i++) {
      const y = Math.random() * 0.7*(canvas.height+0.31*canvas.height)/1.35+0.5*canvas.height;
      if (y < linePosition) {
...
const sigma = 100;
const xOffset = canvas.width / 1.68;
...
function diffractionLeft(){
  ...
  for (let i = 0; i < 10; i++) {
      ...
      const y = Math.random() * t/2 +0.5*canvas.height;
      if (y < linePosition) {
  ...
  const curveLength = 500; // Length of curve to draw in pixels
  const sigma = 900;
  const xOffset = canvas.width / 1.69;
...
function diffractionRight(){
  ...
  for (let i = 0; i < 10; i++) {
      ...
      const y = Math.random() * t2/2 + 0.5*canvas.height;
      if (y < linePosition) {
      ctx.fillStyle = `rgb(255,255,255)`;
      ctx.fillRect( y, x1 + i,1, 1);
      }
...
resetButton.addEventListener("click" , function(){
    ...
    ctx.fillStyle = "white";
    ctx.fillRect(canvas.width * 0.59, 0, 1, canvas.height);
    ctx.fillStyle="grey";
